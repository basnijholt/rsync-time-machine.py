...
SAMPLE_PID = 1234  # Arbitrary PID for testing
...
def test_normalize_pid_variants(rtm: Any) -> None:
    ...
    assert module.normalize_pid(str(SAMPLE_PID), None) == SAMPLE_PID
...
def test_exit_if_pid_running_invokes_ps(
    ...
    def fake_run_cmd(cmd: str, _ssh: Any | None = None) -> Any:
        commands.append(cmd)
...
def test_start_backup_cleans_temp_exclusion_on_failure(
    ...
    def fake_run_cmd(cmd: str, _ssh: Any | None = None) -> Any:
        if cmd.startswith("echo"):
            ...
            return module.CmdResult("", "", 0)
        raise RuntimeError(rsync_error_msg)
...
def test_deal_with_no_space_left_handles_non_utf8(
    ...
    monkeypatch.setattr(module, "find_backups", lambda _dest, _ssh: ["a", "b"])

    expired: list[str] = []

    def fake_expire(folder: str, _ssh: Any) -> None:
        expired.append(folder)

    monkeypatch.setattr(module, "expire_backup", fake_expire)
    monkeypatch.setattr(module, "log_warn", lambda _message: None)
    monkeypatch.setattr(module, "log_error", lambda _message: None)
...
def test_check_rsync_errors_handles_non_utf8_error(
    ...
    assert messages
    assert "rsync error" in messages[0]
